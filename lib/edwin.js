...
Edwin.prototype.converse = function(rawInput) {
  ...
  dialog.startConversation()
    .then(() => dialog.respondIfEmptyStatement())
    .then(() => dialog.respondIfQuitInturrupt())
    .then(() => dialog.respondIfTrivialResponseRequired())
    .then(() => dialog.setInitialIntent())
    .then(() => dialog.setInitialImpliedContext())
    .then(() => dialog.respondIfInvalidIntent())
    .then(() => dialog.setInitialTopic())
    .then(() => dialog.respondIfNoTopic())
    .then(() => dialog.setInitialContextFromModifiers())
    .then(() => dialog.applyAnnotation())
    .then(() => {
