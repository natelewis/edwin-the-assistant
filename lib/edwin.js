...
Edwin.prototype.converse = function (state, rawInput, callback) {
    ...
    if (typeof (state.query) !== 'undefined') {
        ...
        if (fs.existsSync(conversationJSONFile)) {
            return callback(new ConversationHandler(state, callback, jsonfile.readFileSync(conversationJSONFile)));
        } else {
            // if we have an action map end it off to the action handler
            var actionHandler = load.action(state.conversation, this.debug);
    ...
    if (typeof (state.action) !== 'undefined') {
        ...
        if (fs.existsSync(intentJSONFile)) {
            ...
            if (fs.existsSync(annotationJSONFile)) {
                ...
                return callback(new AnnotationHandler(state, callback, jsonfile.readFileSync(annotationJSONFile)));
            }
            // if we have an action map end it off to the action handler
            var actionHandler = load.action(state.conversation, this.debug);
            if (typeof (actionHandler) !== 'undefined') {
