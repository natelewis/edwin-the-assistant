...
Edwin.prototype.converse = function (state, rawInput, callback) {
    ...
    if (typeof (state.query) !== 'undefined') {
        ...
        // new converstation - overwrite action state with a new one based on the new statement
    ...
    if (typeof (state) !== 'undefined') {
        ...
        // run the intent
        var intentHandler = load.intent(state.action, state.context, this.debug);
        if (typeof (intentHandler) !== 'undefined') {
            // pass on the callback to process
            return intentHandler.process(state, callback, this.debug);
        }
    }

    // clean state and figure out how to recover, either we are done
    // or confused
    // state = {};
    ...
    //    state.reply = undefined;
    // }
    // if we don't have a reply let make it up because we won't need it
    if (typeof (state) === 'undefined') {
        state = {};
    }
