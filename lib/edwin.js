...
Edwin.prototype._processResponse = function(convo, callback, debug) {
  if (typeof(convo.state.currentContext) === 'undefined') {
    ...
    if (intent.get()) {
      ...
      convo.state.conversation = intent.updateConversation(convo.state.context);

      // try this one first
      let annotationJSONFile = path.join(__dirname, '..', 'annotation', convo.state.conversation + '.json');
  ...
  // ty this one first
  let conversationJSONFile = path.join(
    __dirname,
    '..',
    'conversation',
    convo.state.conversation + '.json'
  );

  // check if json version is present
  if (fs.existsSync(conversationJSONFile)) {
    let conversationObj = jsonfile.readFileSync(conversationJSONFile);
    return convo.conversationHandler(conversationObj);
  }
