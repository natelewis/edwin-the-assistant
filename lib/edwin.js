  ...
  dialog.checkForQuitResponse()
    ...
    .then( (state) => {
      while (dialog.notResponded()) {
        ...
        dialog.state.exit = undefined;

        // do the conversation logic
        this._processResponse(dialog, callback, this.debug);
      }
...
Edwin.prototype._processResponse = function(dialog, callback, debug) {
  ...
  if (typeof(dialog.state.currentContext) === 'undefined') {
    ...
    if (intent.get()) {
      ...
      dialog.state = new AnnotationHandler(dialog.state, callback, annotationJSON);
    }
  }
  ...
  if (fs.existsSync(conversationJSONFile)) {
    dialog.state = new ConversationHandler(dialog.state, callback, jsonfile.readFileSync(conversationJSONFile));
    return dialog.state;
  }
