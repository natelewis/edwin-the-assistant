...
Edwin.prototype.converse = function(state, rawInput, callback) {
  ...
  convo.checkForQuitResponseInturrupt()
    .then(() => {
...
Edwin.prototype._processResponse = function(convo, callback, debug) {
  if (typeof(convo.state.currentContext) === 'undefined') {
    ...
    if (intent.get()) {
      ...
      this.debug && console.log('topic: ' + convo.state.topic);

      // update the context & topic based on intent
      convo.state.context = intent.updateContext(
        convo.state.context,
        convo.state.statement
      );
      ...
      convo.state = new AnnotationHandler(
        convo.state,
        convo.finish
      );
    }
  ...
  this.debug && console.log('context: ' + convo.state.context);
  ...
  this.debug && console.log('topic file:: ' + topicJSONFile);
  ...
  if (convo.notResponded()) {
    ...
    convo.setFinal('Not sure I understand');
    return convo.finish();
  }
};
