...
Edwin.prototype._replyWithWelcome = function(state, callback) {
  var greeting = ['What\'s up?', 'What\'s going on?', 'Can I help with something?'];
...
Edwin.prototype.converse = function(state, rawInput, callback) {
  ...
  if (typeof(rawInput) !== 'undefined' && rawInput.match(/^(no quit|please quit|leave me|shut up|stop talking|quit).*/i)) {
  ...
  if (typeof(state.action) !== 'undefined') {
    ...
    while (typeof(state.reply) === 'undefined' && typeof(state.final) === 'undefined') {
      // make sure we reset anything that could cause problems if we are coming from
...
Edwin.prototype._processResponse = function(state, callback, debug) {
  if (typeof(state.currentContext) === 'undefined') {
    ...
    let intentObj = intent.get(state.action);
    if (intentObj) {
      state = new IntentHandler(state, callback, intentObj);
