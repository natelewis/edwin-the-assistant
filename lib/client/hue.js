...
const HUE_CONFIG_FILE = './hue.json';
...
class Hue {
  ...
  constructor(options) {
    ...
    if (fs.existsSync(HUE_CONFIG_FILE)) {
      this.config.loadFile(HUE_CONFIG_FILE);
    }
  }
   ...
   */
  saveConfig() {
    fs.writeFileSync(HUE_CONFIG_FILE, JSON.stringify(this.config.get(), null, 2));
  }
  ...
  setGroupLightsBrightness(id = null, brightness = 100) {
    ...
    const self = this;
    const bri = 255*(brightness/100);
    return this.api.setGroupLightState(id, {'on': true, 'bri': bri})
      .then(() => {
        setTimeout(() => {
          self.syncGroups();
        }, 500);
   ...
   * @param {number}  delay How long to delay before making call
   *
  ...
  /**
   * Look up what group number by the name of the group. syncGroups must be
   * called before calling this function.
   * @param  {string} name Non case-sensitve name of group
   *
   * @return {Promise}     Number of light group or string error
   */
   ...
   * @param  {string} name Non case-sensitve name of group
   *
   * @return {object}      Object of light group or string error
   */
  getGroupByNameAsync(name) {
    if (fs.existsSync(HUE_CONFIG_FILE)) {
      this.config.loadFile(HUE_CONFIG_FILE);
    }
    const group = this.config.get('groups');
