...
if (!appConfig.get('standalone.enabled')) {
  ...
  const talkingSpiner = ora({spinner: 'balloon2'});
  const networkSpinner = ora({spinner: 'hamburger'});
  ...
  micToSpeech.micInputStream.on('speaking', () => {
    talkingSpiner.start();
  });

  micToSpeech.micInputStream.on('silence', () => {
    talkingSpiner.stop();
    cliCursor.hide();
  });
      ...
      .then(
        (statement) => {
          ...
          if (activeStandaloneConversation) {
            ...
            // nothing to process just resume the mic
            micToSpeech.resume();
          }
        }
      );
