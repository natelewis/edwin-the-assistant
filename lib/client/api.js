...
const _listenerRespondCallback = function(state, sss) {
  ...
  if (sss.getFinal() !== undefined) {
    ...
    listenerStateStorage[sss.getSessionId()] = undefined;
  } else {
    console.log('api: (listener) >> ' + sss.getReply());
    // save the session to memory
    listenerStateStorage[sss.getSessionId()] = state;
  }

  state.final = sss.getFinal();
  state.reply = sss.getReply();

  // respond to the listener device
  sss.getResponseObject().send(JSON.stringify(state));

  // reset our state if we are done
  if (sss.getFinal() !== undefined) {
    sss.reset();
  }
};
...
module.exports = (function() {
  ...
  if (configAPI.enabled) {
    ...
    api.post('/listener', auth, (req, res, next) => {
      ...
      edwin.converse(state, req.body.statement, _listenerRespondCallback);
    });
