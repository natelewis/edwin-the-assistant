...
class State {
  /**
   * State constructor
   * @constructs Intent returing the common State instance
   */
  constructor() {
    if (!instance) {
      instance = this;
  ...
  /**
   * Callback once setReply or setFinal is done
   * @param {func} value The callback
   */
  setCallback(value) {
    this.callback = value;
    this.save();
  ...
  /**
   * Execute the callback set by setCallback. But only once!
   * @return {Object} Returns the current state
   */
  finish() {
    /*
    if (!this._callbackFired) {
      // set the fired flag so we don't do it more than once
      this._callbackFired = true;
      // send the response to the client asking about it
   ...
   */
  notResponded() {
    //console.log('notResponded', this.getFinal(), this.getReply())
