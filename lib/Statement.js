...
class Statement {
  ...
  getImpliedContext() {
    ...
    if (this.startsWithCanOrWill()) {
      this.context = this.words.getNextWordOfTypeAfterWord(
        this.wordsLC[2],
        'NNP',
        this.debug
      );
    ...
    if (this.hasAnAdjective()) {
      const firstAdjective = this.words.getWordsByType(this.tagged, 'JJ')[0];
      // try NN for context
      this.context = this.words.getNextWordOfTypeAfterWord(
        firstAdjective, 'NN',
        this.debug
  ...
  getImpliedIntent() {
    ...
    if (this.hasAVerb()) {
      let firstVerb = this.words.getWordsByType(this.tagged, 'VB', true)[0];
    ...
    if (this.hasPropositionAndNoun()) {
      this.intent = this.words.getWordsByType(this.tagged, 'NN')[0];
      return this.logIntentDebug(
        'hasPropositionAndNoun',
        this.intent
      );
    ...
    if (this.hasAnAdjective()) {
      this.intent = this.words.getWordsByType(this.tagged, 'JJ')[0];
      return this.logIntentDebug(
        'hasAnAdjective',
        this.intent
      );
