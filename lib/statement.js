...
Statement.prototype._processStatement = function () {
    ...
    } else if (wlc.length > 2 && (wlc[0] === 'can' || wlc[0] === 'will')) {
    ...
    } else if (wlc.length > 2 && (this.wordTypes[0] === 'NN' && this.wordTypes[1].match(/DT/))) {
    ...
    } else if (wlc.length > 1 && (this.wordTypes[0] === 'NN' && this.wordTypes[1].match(/NN/))) {
        // first word is noun followed by another noun
        this.context = wlc[1];
        this.action = wlc[0];
        this.debug && console.log('statement: action match "' + this.action + '" - starts with nown followed by NNP with context of ' + this.context);
    } else if (wlc.length > 2 && (this.wordTypes[0] === 'NN' && this.wordTypes[1] === 'TO')) {
    ...
    } else if (words.getWordsByType(this.tagged, 'NN').length > 0 && words.getWordsByType(this.tagged, 'IN').length > 0) {
        // Prop & nown
    ...
    } else if (words.getWordsByType(this.tagged, 'JJ').length > 0) {
        // last resort... adjective?
        this.action = words.getWordsByType(this.tagged, 'JJ')[0];
        this.debug && console.log('statement: action match "' + this.action + '" - adjective last resort');
    }
