...
Statement.prototype.replaceFields = function(state) {
  ...
  while ((result = swapVarRegex.exec(this.text)) !== null) {
    ...
    replacedReply = replacedReply.replace(
      result[0],
      (state[result[1]] === undefined) ? '' : state[result[1]]
    );
  }
...
Statement.prototype._processStatement = function() {
  ...
  if (wlc.length > 2 && (this.wordTypes[0] === 'NN' && this.wordTypes[1].match(/DT/))) {
    ...
    this.context = wlc[2];
    this.action = wlc[0];
    this.debug && console.log('statement: action match "' + this.action + '" - starts with noun followed by DT with context of ' + this.context);
    return this.action;
  }
  ...
  if (wlc.length > 1 && (this.wordTypes[0] === 'NN' && this.wordTypes[1].match(/NN/))) {
    ...
    this.context = wlc[1];
    this.action = wlc[0];
    this.debug && console.log('statement: action match "' + this.action + '" - starts with noun followed by NNP with context of ' + this.context);
    return this.action;
  }
  ...
  if (wlc.length > 2 && (this.wordTypes[0] === 'NN' && this.wordTypes[1] === 'TO')) {
    ...
    this.context = wlc[2];
    this.action = wlc[0];
    this.debug && console.log('statement: action match "' + this.action + '" - starts with noun followed by TO with context of ' + this.context);
    return this.action;
  }
  ...
  if (words.getWordsByType(this.tagged, 'NN').length > 0 && words.getWordsByType(this.tagged, 'IN').length > 0) {
    ...
    this.debug && console.log('statement: action match "' + this.action + '" - proposition & noun, noun action');
    return this.action;
  }
