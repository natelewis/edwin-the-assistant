...
ActionHandler.prototype._processHandler = function () {
    ...
    for (var fieldObj in fields) {
        if (fields.hasOwnProperty(fieldObj)) {
            var randomReplyIndex = Math.floor(Math.random() * fields[fieldObj].reply.length);
            var field = fields[fieldObj].field;
            var type = fields[fieldObj].type;
            var reply = fields[fieldObj].reply[randomReplyIndex];
            ...
            if (this.state.query === field) {
                ...
                this.state = new Validate(this.state, fields[fieldObj]);
            }
