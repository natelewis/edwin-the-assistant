...
/*
    // try this one first
    var annotationJSONFile = path.join(__dirname, '..', 'annotation', this.state.conversation + '.json');

        // check if json version is present
    if (fs.existsSync(annotationJSONFile)) {
        return new AnnotationHandler(this.state, this.callback, jsonfile.readFileSync(annotationJSONFile));
    }
