...
IntentHandler.prototype._processIntent = function () {
    ...
    if (typeof (this.state.originalContext) === 'undefined') {
        ...
        var moduleMap = this.intent.moduleMap;
        for (var moduleKey in moduleMap) {
            var intent = moduleMap[moduleKey];
            if (this.state.context === intent.context) {
                this.state.module = intent.module;
            }
        }
    }
        // try this one first
    var conversationJSONFile = path.join(__dirname, '..', 'conversation', this.state.module + '.json');
