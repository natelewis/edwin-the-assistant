  ...
  */
  ...
  conversationHandler(sss) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      // hold on to topic from what it was to make sure we know what it was
      // for any logic we need if it changes
      const preModuleTopic = sss.getTopic();

      // topic steps
      let steps = topic.steps();

      // processConversationStep(steps)
      ...
      while (steps.length > 0) {
        ...
        // if we have not responded yet, keep on processing steps
        // if (sss.notResponded()) {
          ...
          if (step.module !== undefined && sss.notResponded()) {
            ...
            const module = load.module(step.module, self.debug);
            if (typeof(module) === 'undefined') {
              ...
              module.run(sss, step.config).then(() => resolve());
            }
          } else {
            resolve(sss);
          }
