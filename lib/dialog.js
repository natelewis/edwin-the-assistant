...
class Dialog {
  ...
  constructor(config) {
    ...
    if (typeof(config.state) === 'undefined' &&
        typeof(config.rawInput) !== 'undefined' &&
        config.rawInput.split(' ').length > 0) {
      config.state = this.applyStatementToState({}, config.rawInput, config.debug);
    }
    // Peristent vars
   ...
   * @return {[type]}       [description]
   */
  applyStatementToState(state, text, debug) {
    ...
    if (typeof(statement.action) === 'undefined') {
      ...
      state = {
        ...
        initialStatement: state.rawInput,
      };
    }
    return state;
  }
   ...
   */
  isConverstationQuit(state) {
    let rawInput = state.rawInput;
    if (rawInput.match(/^(no quit|please quit|leave me|shut up|stop talking|quit).*/i)) {
      return true;
    };
    return false;
  }
  ...
  replyWithQuit(state) {
    ...
    return new Promise( function( resolve, reject) {
      ...
      if (self.isConverstationQuit(state)) {
