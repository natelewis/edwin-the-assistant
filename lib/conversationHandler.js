...
ConversationHandler.prototype._processHandler = function () {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (testConditional(step.requirement, this.state, this.debug)) {
...
function testConditional (requirement, state, debug) {
    if (typeof (requirement) !== 'undefined') {
        for (let x = 0; x < requirement.length; x++) {
            let cond = requirement[x];

            debug && console.log(cond.type + '  ' + cond.field + '  ' + cond.operator + '  ' + cond.value);
            if (cond.type === 'typeof') {
                if (cond.operator === '!==') {
