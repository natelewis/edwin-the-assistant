...
Api.prototype.handler = function (req, res) {
    if (req.url === '/api/intents') {
        ...
        fs.readdir(this.conversationFolder, (err, files) => {
            if (err) {
                res.status(404).send('Error accessing converstations');
                return;
            }
            files.forEach(file => {
                if (file.endsWith('.json')) {
        ...
        if (conversation.match(lettersOnly)) {
            ...
            if (req.method === 'GET') {
                ...
                this._readConverstationJSON(conversation, function (err, data) {
                    if (err) {
                        console.log(err);
                        res.status(404).send('Converstation not found');
                        return;
                    }
                    res.send(data);
                });
            ...
            if (req.method === 'POST') {
                ...
                this._writeConverstationJSON(conversation, JSON.stringify(req.body, null, '    '), function (err) {
                    if (err) {
                        console.log(err);
                        res.status(404).send('Converstation not saved');
                        return;
                    }
                    res.send('Converstation saved');
                });
            }
        } else {
            res.status(404).send('Not found');
...
Api.prototype._readConverstationJSON = function (conversation, callback) {
    fs.readFile(this.conversationFolder + conversation + '.json', 'utf-8', callback);
};

Api.prototype._writeConverstationJSON = function (conversation, json, callback) {
