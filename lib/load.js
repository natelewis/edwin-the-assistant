...
module.exports = {
    ...
    intent: function (intentName, context, debug) {
        ...
        try {
            ...
            if (e instanceof Error && e.code === 'MODULE_NOT_FOUND') {
                ...
                debug && console.log('load: Intent ' + intentName + ' has errors');
                debug && console.log(e);
                throw e;
            }
            return undefined;
    ...
    // dynamicly pull in an action
    action: function (actionName, debug) {
        var modulePath = '../action/' + actionName;

        try {
            var module = require(modulePath);
        } catch (e) {
            if (e instanceof Error && e.code === 'MODULE_NOT_FOUND') {
                debug && console.log('load: Action ' + actionName + ' not available');
            } else {
                throw e;
            }
            return undefined;
