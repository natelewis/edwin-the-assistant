...
function Words (text) {
    ...
    this.wordTypeNames = [];
    this.wordTypes = [];
    for (var i = 0; i < this.wordsLC.length; i++) {
...
Words.prototype.getEverythingBeforeWord = function (startWord) {
    ...
    var startIndex = this.wordsLC.indexOf(startWord.toLowerCase()) - 1;

    // all the words joined form the words we have
    var allTheWords = this.words.slice(0, startIndex).join(' ');
...
Words.prototype.getEverythingAfterWord = function (startWord) {
    ...
    // pull the words
    var allTheWords = this.words.slice(startIndex, this.words.length).join(' ');
...
Words.prototype.getWordTypeName = function (singleWord) {
    if (singleWord) {
        var tag = tagger.tag(lexer.lex(singleWord))[0][1];
...
Words.prototype.getNextWordOfTypeAfterWord = function (startWord, nextType) {
    var startIndex = this.wordsLC.indexOf(startWord.toLowerCase());
    var arrayLength = this.wordsLC.length;
