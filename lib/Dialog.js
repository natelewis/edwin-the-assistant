...
class Dialog {
   ...
   */
  startConversation(sss, statement) {
    return new Promise(function(resolve, reject) {
      // set the statment to state
      sss.setStatement(statement);
  ...
  respondIfQuitInturrupt(sss) {
    if (sss.notResponded()) {
      ...
      if (this.isConversationQuit(sss)) {
  ...
  processStep(steps, sss) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (sss.getField(step.query) === undefined && sss.notResponded()) {
        ...
        self.setReplyIfAvailable(step, sss);
      }
